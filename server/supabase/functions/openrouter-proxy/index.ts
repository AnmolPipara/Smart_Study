...
async function callOpenRouter(prompt: string): Promise<{content: string; provider: string}> {
  ...
  const response = await fetch(OPENROUTER_URL, {
    ...
    body: JSON.stringify({ model: "google/gemma-4-31b-it:free", messages: buildMessages(prompt), temperature: 0.4 }),
  });
...
async function callGroq(prompt: string): Promise<{content: string; provider: string}> {
  ...
  const response = await fetch(GROQ_URL, {
    ...
    body: JSON.stringify({ model: "llama-3.3-70b-versatile", messages: buildMessages(prompt), temperature: 0.4 }),
  });
