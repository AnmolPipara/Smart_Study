...
/** Returns true if the last AI call used demo mode (no live provider available). */
export function isDemoMode(): boolean {
  return _isDemoMode;
}

/** Returns the name of the provider used in the last AI call (e.g. "openrouter", "groq", "gemini", "demo"). */
...
async function callOpenRouter(prompt: string): Promise<string> {
  ...
  // Extract demo mode metadata from backend
  if (data?._meta) {
    _isDemoMode = data._meta.demo_mode === true;
    _lastProvider = data._meta.provider || '';
  } else {
    _isDemoMode = false;
    _lastProvider = 'openrouter';
  }
